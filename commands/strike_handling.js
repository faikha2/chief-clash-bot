...
function add_strike(client, name, reason) {
    ...
    // Get a list of the names from our data file
    var usernames = []
    var rel_username = ""
    for (key in strike_data) {
      ...
      if (strike_data[key].id == name) {
        ...
        if (strike_data[key].num_strikes === 3) {
          ...
          // A formatted Discord embed. Makes the output look beautiful in our Discord channel! 
          ...
          // Sends the embed to our channel
          channel.send(exampleEmbed);

          // We break here because 3 is the max number of strikes. If someone has 3, we do not do anything
          break;
        }
        ...
        if (!strike_data[key].reasons[0].reason_1) {
          ...
          // Sends confirmation to channel and saves reason into variable
          ...
          // Sends the embed to our target channel
          channel.send(exampleEmbed);
        }
    ...
    // This happens if we go through the loop and do not find the given username
    if (rel_username == "") {
